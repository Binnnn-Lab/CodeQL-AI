...
1. Compiles the **real source file** (with `-g -O0 -fno-inline`) and links it
   into the harness, so symbolic execution runs over actual sanitizer code.
...
| `build_harness` | `source_file`, `vuln_entry`, `source_api`, `compile_script`, `dataset_path` | `{binary_path, harness_path, dwarf_ok: bool, source_mode: "libc_stdin"\|"mid_function"\|"entry_fallback"}` | No (template only) |
...
for each path in paths:
    build_harness(...)             → binary_path       (deterministic)
...
Replaces the previous `generate_harness` + `compile_harness`. The harness is
**a fixed template** — no agent input beyond which function to call:

```c
#include <stdio.h>
extern void vuln_entry(void);     /* resolved at link time from original .c */
void __sink_reached(void) {}
int main(void) { vuln_entry(); return 0; }
```

Where `vuln_entry` is the actual name of the SARIF source-path's containing
function. If that function has a non-void signature, `build_harness` emits an
adapter wrapper with the matching `extern` declaration and supplies dummy
arguments (zero-initialised) — its job is only to get execution into the
function so angr can take over.

Where `vuln_entry` is the SARIF source-path's containing function (e.g.
Juliet's `goodB2G`). The harness `.c` is compiled **alongside the original
source file**; the linker resolves `vuln_entry`. compile.sh enforces
`-g -O0 -fno-inline` so DWARF is reliable.
...
- `libc_stdin` — SARIF source-node API name is in the built-in libc list
  ...
  Runtime uses `full_init_state(stdin=SimFileStream(content=BVS))`.
...
src/libs/symbolic_sanitizer/
  ...
  harness_builder.py     # NEW: replaces harness_generator.py
                         # build_harness(), compile.sh template, source_mode selection
...
```
bash compile.sh <harness_src> <original_src> <output_binary> <lang>
```

(adds `<original_src>` — the SARIF source file that defines `vuln_entry`.)
...
"$CC" $STD -g -O0 -fno-inline \
  ...
  "$HARNESS" "$ORIG" -o "$OUT" -lm 2>&1
```

`-g -O0 -fno-inline` is non-negotiable; the DWARF resolver depends on it. If
the dataset's compile.sh already exists but lacks these flags, `build_harness`
...
| Original source missing on disk | `build_harness` fails fast with clear error |
| compile.sh missing | `build_harness` fails fast with instruction to write one |
| compile.sh produces binary without DWARF | `build_harness` returns `dwarf_ok: false`; downstream tools use `entry_fallback`, results marked `degraded: "no_dwarf"` |
...
- `harness_builder`: template renders, source_mode selection table, missing
